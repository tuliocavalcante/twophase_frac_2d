...
%       C(active, i-1+(1:opt.substeps)) = repmat(pos(active, 1), [1, opt.substeps]);
      ...
      active(active)    =  and(pos(active,1) ~= current_cell, ...
                            ~any(abs(bsxfun(@minus,current_cell, prd))<1e-6,2)) ;
...
% In addition, compute curve within cell.
   ...
   global elemarea elem coord 

   f = elemfluxedge(pos(:,1),:);
   n = neigh(pos(:,1),:);
   
       % Coordenadas exatamente no vertice
       
%        for i = 1:size(pos,1)
%            if (pos(i,2) ==1 && pos(i,3)==0)
%                if f(i,2)>f(i,3)
%                    pos(i,3) = pos(i,3)+ 0.01;
%                elseif f(i,3)>= f(i,3)
%                    pos(:,2) = pos(i,2)- 0.01;
%                end
%            elseif (pos(i,2) ==0 && pos(i,3)==1)
%                if f(i,1)>f(i,4)
%                    pos(i,3) = pos(i,3)- 0.01;
%                elseif f(i,4)>= f(i,1)
%                    pos(:,2) = pos(i,2)+ 0.01;
